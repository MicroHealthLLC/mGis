<template>
  <div id="facility_sidebar">
    <div class="row">
      ...
      <!-- Kanban Starts here -->
      <div
        class="col-md-10 facility-show-tab pr-3"
        data-cy="sheets_view"
        style="background-color: solid #ededed 15px"
        v-if="isKanbanView && !getRiskFormOpen"
      >
        <custom-tabs
          :current-tab="currentTab"
          :tabs="tabs"
          @on-change-tab="onChangeTab"
          class="tab pt-3 float-left"
          badge-margin="4px"
        />
        <div class="d-inline align-item-center kanban-filters-bar w-100">
          <div v-if="currentTab === 'tasks'">
            ...
            <div
              class="simple-select w-25 d-inline mr-1"
            ...
            <div
              class="simple-select w-25 d-inline"
              style="position: absolute; right: 23.8%"
            >
          ...
          <div v-if="currentTab === 'issues'">
            ...
            <div
              class="simple-select w-25 mr-1 d-inline"
              style="position: absolute"
            >
              <label class="font-sm mb-0">Category</label>
              <el-select
...
export default {
  ...
  mounted() {
    ...
    if (
      this.getPreviousRoute === "ProjectMapView" &&
  ...
  computed: {
    ...
    filteredTasks() {
      ...
      return _.orderBy(
        _.filter(this.currentFacility.tasks, (resource) => {
      ...
      set(value) {
        if (value)
          this.setMyActionsFilter([
            ...this.myActionsFilter,
            { name: "My Tasks", value: "tasks" },
          ]);
        else
          this.setMyActionsFilter(
            this.myActionsFilter.filter((f) => f.value !== "tasks")
          );
      },
    },
    C_taskTypeFilter: {
    ...
    filteredIssues() {
      ...
      return _.orderBy(
        _.filter(this.currentFacility.issues, (resource) => {
          let valid = Boolean(resource && resource.hasOwnProperty("progress"));
    ...
    C_myIssues: {
      ...
      set(value) {
        if (value)
          ...
          this.setMyActionsFilter(
            this.myActionsFilter.filter((f) => f.value !== "issues")
          );
      },
    },
    C_onWatchIssues: {
      ...
      set(value) {
        if (value)
          ...
          this.setOnWatchFilter(
            this.onWatchFilter.filter((f) => f.value !== "issues")
          );
      },
    },
    ...
    filteredRisks() {
      ...
      return _.orderBy(
        _.filter(this.currentFacility.risks, (resource) => {
          let valid = Boolean(resource && resource.hasOwnProperty("progress"));
    ...
    C_myRisks: {
      ...
      set(value) {
        if (value)
          this.setMyActionsFilter([
            ...this.myActionsFilter,
            { name: "My Risks", value: "risks" },
          ]);
        else
          this.setMyActionsFilter(
            this.myActionsFilter.filter((f) => f.value !== "risks")
          );
      },
    },
    C_onWatchRisks: {
      ...
      set(value) {
        if (value)
          this.setOnWatchFilter([
            ...this.onWatchFilter,
            { name: "On Watch Risks", value: "risks" },
          ]);
        else
          this.setOnWatchFilter(
            this.onWatchFilter.filter((f) => f.value !== "risks")
          );
      },
    },
    filterTaskStages() {
    ...
    C_onWatchTasks: {
      ...
      set(value) {
        if (value)
          ...
          this.setOnWatchFilter(
            this.onWatchFilter.filter((f) => f.value !== "tasks")
          );
      },
    },
    C_kanban() {
    ...
    filteredFacilityGroups: {
      handler(value) {
        if (
          !(
            this.currentFacilityGroup &&
            _.map(value, "id").includes(this.currentFacilityGroup.id)
          )
        ) {
          this.currentFacilityGroup = {};
          this.currentFacility = {};
          ...
          if (group) {
            this.currentFacilityGroup = group;
            let facility = this.facilityGroupFacilities(
              this.currentFacilityGroup
            ).find((f) => f.id == this.currentFacility.id);
            if (facility) {
              this.currentFacility = facility;
            } else {
              this.currentFacility = {};
        ...
        if (this.currentFacility && !this.currentFacility.id) {
          if (Vue.prototype.$preferences.project_id) {
            var facility = this.facilityGroupFacilities(
              this.currentFacilityGroup
            ).find((f) => f.id == Vue.prototype.$preferences.project_id);
            if (facility) {
              this.currentFacility = facility;
            }
          }
...
#facility_sidebar {
  ...
  .center-section {
    position: absolute;
    box-shadow: 0.5px 0.5px 5px 5px rgba(0, 0, 0, 0.19),
